...
void scale_horizontal_fit(sf::RenderWindow* render_window, sf::Sprite* wallpaper_sprite)
{
  ...
  float scale = static_cast<float>(static_cast<float>(render_window_size.x)) / static_cast<float>(wallpaper_texture_size.x);
  wallpaper_sprite->setScale(scale, scale);

  float y_offset = (static_cast<float>(render_window_size.y) - (static_cast<float>(wallpaper_texture_size.y) * scale)) / 2;
...
void scale_vertical_fit(sf::RenderWindow* render_window, sf::Sprite* wallpaper_sprite)
{
  ...
  float scale = static_cast<float>(static_cast<float>(render_window_size.y)) / static_cast<float>(wallpaper_texture_size.y);
  wallpaper_sprite->setScale(scale, scale);

  float x_offset = (static_cast<float>(render_window_size.x) - (static_cast<float>(wallpaper_texture_size.x) * scale)) / 2;
...
void scale_stretched(sf::RenderWindow* render_window, sf::Sprite* wallpaper_sprite)
{
  ...
  float x_scale = static_cast<float>(static_cast<float>(render_window_size.x)) / static_cast<float>(wallpaper_texture_size.x);
  float y_scale = static_cast<float>(static_cast<float>(render_window_size.y)) / static_cast<float>(wallpaper_texture_size.y);
